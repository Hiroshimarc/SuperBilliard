...
public class Balle extends Circle{
     ...
     */
    public void launch(Balle balle,double angle,double vitesse, Pane pane){ //Remplacer pane par la table
        ...
        new Thread(()->{
            ...
            while(!Thread.interrupted()){
                ...
                if (collisionsTable(balle, pane)[0] || collisionsTable(balle, pane)[1]) {
                    varX *= -1;
                }

                if (collisionsTable(balle, pane)[2] || collisionsTable(balle, pane)[3]) {
                    varY *= -1;
                }

                if(Math.sqrt(Math.pow(varX, 2)+Math.pow(varY, 2)) < 0.0025){
                    varX=0;
                    varY=0;
     ...
     */
    public boolean[] collisionsTable(Balle balle, Pane pane){ //Remplacer pane par table
        boolean[] tabConditionsTable = new boolean[4];
        Bounds limits = pane.getLayoutBounds();
        boolean atRightBorder = balle.getLayoutX() >= (limits.getMaxX() - balle.getRadius());
        boolean atLeftBorder = balle.getLayoutX() <= (limits.getMinX() + balle.getRadius());
        boolean atBottomBorder = balle.getLayoutY() >= (limits.getMaxY()- balle.getRadius());
        boolean atTopBorder = balle.getLayoutY() <= (limits.getMinY() + balle.getRadius());
    ...
    public double decelerationBalle(double axeVariation){ //Rajouter la masse et le frottement dans les prochaines versions
        axeVariation = 0.9995*axeVariation;
        return axeVariation;
    }
