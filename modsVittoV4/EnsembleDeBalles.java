...
public class EnsembleDeBalles extends BalleStructure {
    ...
    public static boolean collisionDetectee = false;
    ...
    public EnsembleDeBalles(Table surfaceDeJeu) {
        ...
        balleHuit = new BalleStructure(new Balle(8, 15, Color.BLACK, 2.295 * surfaceDeJeu.getBoundsInLocal().getWidth() / 3, surfaceDeJeu.getBoundsInLocal().getHeight() / 2), 0.162);
        ...
        //Balles avec motifs
        balle9 = new BalleStructure(new Balle(9, 15, Color.RED, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 60), 0.162);
        balle10 = new BalleStructure(new Balle(10, 15, Color.BLUE, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 90), 0.162);
        balle11 = new BalleStructure(new Balle(11, 15, Color.YELLOW, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 120), 0.162);
        balle12 = new BalleStructure(new Balle(12, 15, Color.CYAN, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 150), 0.162);
        balle13 = new BalleStructure(new Balle(13, 15, Color.PURPLE, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 180), 0.162);
        balle14 = new BalleStructure(new Balle(14, 15, Color.CHARTREUSE, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 210), 0.162);
        balle15 = new BalleStructure(new Balle(15, 15, Color.DARKGOLDENROD, surfaceDeJeu.getLayoutBounds().getMinX() + 50, 240), 0.162);

        //Balles sans motifs
        balle1 = new BalleStructure(new Balle(1, 15, Color.RED, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 60), 0.162);
        balle2 = new BalleStructure(new Balle(2, 15, Color.BLUE, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 90), 0.162);
        balle3 = new BalleStructure(new Balle(3, 15, Color.YELLOW, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 120), 0.162);
        balle4 = new BalleStructure(new Balle(4, 15, Color.CYAN, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 150), 0.162);
        balle5 = new BalleStructure(new Balle(5, 15, Color.PURPLE, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 180), 0.162);
        balle6 = new BalleStructure(new Balle(6, 15, Color.CHARTREUSE, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 210), 0.162);
        balle7 = new BalleStructure(new Balle(7, 15, Color.DARKGOLDENROD, surfaceDeJeu.getLayoutBounds().getMaxX() - 50, 240), 0.162);
     ...
     *
     * @param table Table sur laquelle se trouvent les BalleStructure
     */
    ...
    /**
     * Méthode qui permet de traiter les collisions avec la nouvelle table
     *
     * @param list Liste de coordonnées et de Path qui délimitent la surface de
     * jeu
     */
     ...
     * m1u1+m2u2 = m1v1+m2v2
     *
     * @param table Surface de jeu (Objet de type Table) PROBLÈME DÉTECTÉ:
     * Création anormale d'énergie lors du transfert de la vitesse
     */
    public void observeBalles(Table table) {
        Thread tCB = new Thread() {
            @Override
            public void run() {
                    if (tabBalles[0].getBoundsInParent().intersects(tabBalles[1].getBoundsInParent()) && tabBalles[0].obtenirVitesseActuelle() != 0) {
                        //On conserve en mémoire la vitesse de la tabBalles[i]
                        double vitesseCollision = tabBalles[0].obtenirVitesseActuelle();
                        System.out.println("vitesse actuelle: "+ vitesseCollision);
                        //On conserve en mémoire l'angle auquel relancer la balle lancée
                        double angleF = angleDeviation(tabBalles[0], tabBalles[1], table);
                        //On conserve en mémoire le momentum Piy
                        double piY = tabBalles[0].getVarY() + tabBalles[1].getVarY();
                        //On conserve en mémoire le momentum Pix
                        double piX = tabBalles[0].getVarX() + tabBalles[1].getVarX();
                        //On conserve en mémoire le momentum pfYA
                        double pfYA = vitesseCollision * (Math.sin(angleF));
                        //On conserve en mémoire le momentum pfYA
                        double pfXA = vitesseCollision * (Math.cos(angleF));
                        //Lancement des balles
                        tabBalles[0].prepareToLaunch(angleF, vitesseCollision);
                        iniBalleFrappee(tabBalles[1], piX, piY, pfXA, pfYA,angleF);
                        try {
                            sleep(100);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(EnsembleDeBalles.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
            }
        };
    ...
    public void ballesInactives() {
        ...
        for (BalleStructure tabBalle : tabBalles) {
            if (tabBalle.obtenirVitesseActuelle() < 0.009) {
                tabBalle.setVarX(0);
                tabBalle.setVarY(0);
     ...
     * déviée
     *
     * @param balleA Premier objet BalleStructure
     * @param balleB Second objet BalleStructure
     * @param table Objet Table représentant la surface de jeu
     * @return Angle de déviation suite à une collision
     */
    public double angleDeviation(BalleStructure balleA, BalleStructure balleB, Table table) {
        ...
        return Math.atan(pente);
    }

    /**
     * Méthode qui calcule la vitesse de la balle frappée
     *
     * @param balle Objet BalleStructure percuté
     * @param piX Momentum 2D initial en composantes X
     * @param piY Momentum 2D initial en composante Y
     * @param pfXA Momentum 2D final de l'objet BalleStructure percuté en
     * composantes X
     * @param pfYA Momentum 2D final de l'objet BalleStructure percuté en
     * composantes Y
     */
    public void iniBalleFrappee(BalleStructure balle, double piX, double piY, double pfXA, double pfYA, double angleF) {
        //Calcul de la composante en Y de la vitesse recherchée
        double vy = (piY - pfYA);
        //Calcul de la composante en X de la vitesse recherchée
        double vx = (piX - pfXA);
        //Ajustement de l'angle pour éviter les déviations anormales
        double angleFinal = Math.toRadians(90)+angleF;
        double vitessef = Math.sqrt(Math.pow(vx, 2) + Math.pow(vy, 2));
        boolean zoneOkay=false;
        if(angleFinal>=Math.toRadians(90) && angleFinal<=Math.toRadians(180)){
            zoneOkay = true;
        }
        //Quadrants
        if(angleFinal>Math.toRadians(0) && angleFinal<=Math.toRadians(90)){
            angleF=angleF-Math.toRadians(180);
        }
        if(angleFinal>Math.toRadians(90) && angleFinal<=Math.toRadians(180)){
            angleF=angleF-Math.toRadians(180);
        }
        if(angleFinal>Math.toRadians(180) && angleFinal<=Math.toRadians(270)){
            angleF=angleF;
        }
        if(angleFinal>Math.toRadians(270) && angleFinal<=Math.toRadians(360)){
            angleF=angleF-Math.toRadians(360);
        }
        System.out.println("Vitesse BalleFrappée: "+vitessef);
        balle.prepareToLaunch(angleFinal, vitessef);
    }
     ...
     * @param pathList Liste de coordonnées et de segments qui délimite la
     * surface de jeu
     */
